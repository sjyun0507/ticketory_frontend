...
export default function MyReveiw() {
  const navigate = useNavigate();
  const storeSnap = typeof useAuthStore?.getState === 'function' ? useAuthStore.getState() : {};
  const memberId = storeSnap?.user?.memberId || storeSnap?.memberId || storeSnap?.profile?.memberId || null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError('');
      try {
        // getMyStories는 Page 형태를 반환한다고 가정
        const res = await getMyStories(memberId, { page, size: PageSize, sort: 'RECENT' });
  ...
  const keyOf = (s, idx) => s?.id ?? s?.storyId ?? s?.uuid ?? `${s?.movieId ?? 'm'}-${s?.memberId ?? 'u'}-${s?.createdAt ?? idx}`;

  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold">내 관람평</h1>
        <button onClick={() => navigate('/story')} className="rounded-lg border px-3 py-1.5 text-sm hover:bg-indigo-50">스토리 피드로</button>
      </div>
        ...
        {stories.map((s, idx) => {
          ...
          return (
            <div key={keyOf(s, idx)} className="grid grid-cols-[80px,1fr] gap-4 p-4">
              <img src={s.posterUrl || s.movie?.posterUrl} alt="poster" className="h-24 w-16 rounded object-cover" />
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {isEditing ? (
                      <>
                        <button onClick={() => applyUpdate(id)} className="rounded-lg bg-indigo-600 px-3 py-1.5 text-sm text-white hover:bg-indigo-700">저장</button>
                        <button onClick={cancelEdit} className="rounded-lg border px-3 py-1.5 text-sm hover:bg-neutral-50">취소</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => startEdit(s)} className="rounded-lg border px-3 py-1.5 text-sm hover:bg-neutral-50">수정</button>
                        <button onClick={() => removeStory(id)} className="rounded-lg border px-3 py-1.5 text-sm text-red-600 hover:bg-red-50">삭제</button>
                      </>
                    )}
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <label className="flex grow items-center gap-2">
                        ...
                        <input type="text" className="grow rounded border px-2 py-1"
                               placeholder="예: 액션, 감동"
                               value={draft.tags}
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-center gap-2">
          <button disabled={page <= 0}
                  ...
                  className="rounded-md border px-3 py-1.5 text-sm disabled:opacity-40">이전</button>
          <div className="text-sm text-neutral-600">{page + 1} / {totalPages}</div>
          <button disabled={page >= totalPages - 1}
                  onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))}
                  className="rounded-md border px-3 py-1.5 text-sm disabled:opacity-40">다음</button>
        </div>
      )}
    </div>
  );
